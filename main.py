...
# قائمة الفيديوهات (يمكنك إضافة حزم تحتوي على أكثر من file_id في حقل files)
VIDEOS = [
...
# 1. استخراج الـ file_id (يدعم الفيديوهات الفردية أو المجموعات)
...
# 3. إرسال الوسائط المدفوعة (فردي أو مجموعة فيديوهات مقفلة)
@bot.callback_query_handler(func=lambda call: call.data.startswith("buy_"))
def buy(call):
    index = int(call.data.split("_")[1])
    item = VIDEOS[index]
    
    # تحويل عناصر القائمة إلى InputMediaVideo لتكوين الحزمة أو الفيديو الواحد
